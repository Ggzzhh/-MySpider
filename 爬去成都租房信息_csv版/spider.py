...
from 爬去成都租房信息_csv版.c_ip import use_proxy, new_identity, get_ips
from 爬去成都租房信息_csv版.models import Throttle

import requests

# 默认代理
PROXY = '101.201.79.172:808'
    ...
    def __call__(self, url):
        ...
        if self.cache:
            ...
            try:
                ...
                # 已经下载了，查看响应码
                if self.num_retries > 0 and (result['code'] is None ):
        ...
        print('结束时间:{}, 总耗时:{}'.format(end_time, end_time-start_time))
        print('-----------------本次结束！--------------------')
        return result
